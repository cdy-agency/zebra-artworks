...
export default function ConstructionDetailPage() {
    ...
    return (
      <main className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-white/40 text-type-prose font-light tracking-widest uppercase">
            Loading project
          </p>
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-background">
      {/* ── Hero ── */}
      <section className="relative w-full h-[55vh] sm:h-[70vh] overflow-hidden bg-[#0a0a0a]">
        <div className="absolute top-0 left-0 right-0 h-0.75 bg-primary z-10" />
        <div className="absolute top-0 left-0 w-0.75 h-full bg-primary z-10" />
        ...
        <div className="absolute top-6 left-8 z-10">
          <Link
            href="/constructions"
            className="inline-flex items-center gap-2 text-white/60 text-type-eyebrow uppercase tracking-widest hover:text-white transition-colors"
          >
            <svg width="12" height="12" viewBox="0 0 14 14" fill="none">
              <path
        ...
        {/* Counter */}
        {images.length > 1 && (
          <div className="absolute bottom-8 right-6 sm:right-10 lg:right-20 z-10 text-white/35 text-type-eyebrow tracking-widest">
      ...
      {images.length > 1 && (
        <motion.section
          ...
          className="px-6 sm:px-10 lg:px-20 py-3 bg-[#0a0a0a] flex gap-2 overflow-x-auto border-b border-white/5"
        >
          {images.map((img, i) => (
            <button
              key={i}
          ...
          <button
            ...
            className="shrink-0 w-20 h-14 sm:w-28 sm:h-20 bg-[#111] border border-white/8 flex flex-col items-center justify-center gap-1 hover:bg-[#1a1a1a] transition-colors focus:outline-none"
          >
            ...
            <span className="text-white/35 text-type-eyebrow uppercase tracking-widest">
              Expand
            </span>
          </button>
        </motion.section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-20">
          ...
          >
            <div className="border-t-[3px] border-primary pt-6 sticky top-24">
              ...
              {/* Stats strip */}
              <div className="mt-8 grid grid-cols-2 bg-subtle">
                <div className="p-4 border-r border-foreground/8 text-center">
                  <p className="text-foreground text-type-h3 font-bold">
                    {images.length}
                  </p>
                  <p className="text-gray-mid text-type-eyebrow uppercase tracking-widest mt-1">
                    Images
                  </p>
                </div>
      ...
      <AnimatePresence>
        {lightbox && (
          ...
          >
            ...
            >
              <p className="text-white/35 text-type-eyebrow uppercase tracking-widest">
                {String(activeImage + 1).padStart(2, "0")} /{" "}
                {String(images.length).padStart(2, "0")}
              </p>
              <button
                onClick={() => setLightbox(false)}
                className="text-white/45 hover:text-white transition-colors text-type-eyebrow uppercase tracking-widest flex items-center gap-2"
              >
                Close
              ...
              {images.length > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-4 sm:left-8 text-white/45 hover:text-white transition-colors p-2"
                  >
                  ...
                  <button
                    ...
                    className="absolute right-4 sm:right-8 text-white/45 hover:text-white transition-colors p-2"
                  >
                    <svg width="20" height="20" viewBox="0 0 14 14" fill="none">
                      <path
                        d="M1 7h12M8 3l5 4-5 4"
